                   ...
                   .content}
{
    try {
        ...
        nhlog::db()->debug("MemberList: room {} has member_count={} from cache",
                          room_id_.toStdString(), info_.member_count);
    } catch (const lmdb::error &e) {
        nhlog::db()->warn("failed to retrieve room info from cache: {} ({})", room_id_.toStdString(), e.what());
    }
    ...
    try {
        ...
        nhlog::db()->debug("MemberList: loaded {} members from cache for room {}",
                          members.size(), room_id_.toStdString());
        addUsers(members);
