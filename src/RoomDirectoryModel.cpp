...
{
        connect(this,
                &RoomDirectoryModel::fetchedRoomsBatchFromMtxClient,
                this,
...
RoomDirectoryModel::joinRoom(const int &index)
{
        ...
        if (index == delegateNoLongerValid)
                return;
...
RoomDirectoryModel::fetchMore(const QModelIndex &)
{
        ...
        http::client()->post_public_rooms(
          req,
          [this, req](const mtx::responses::PublicRooms &res, mtx::http::RequestErr err) {
                  if (err) {
                          nhlog::net()->error(
                          ...
                          emit fetchedRoomsBatchFromMtxClient(
                            res.chunk,
                            res.prev_batch,
                            res.next_batch /*, res.total_room_count_estimate*/);
                  }
          },
          server_);
}
